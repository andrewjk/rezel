...
nonisolated(unsafe) private let htmlElementContext = ContextTracker(
    start: Optional<Any>.none as Any,
...
nonisolated(unsafe) private let htmlHighlighting = styleTags([
...
nonisolated(unsafe) public let htmlParser: LRParser = {
    let parser = try! buildParser(htmlGrammarText, options: BuildOptions(
        externalTokenizer: makeHtmlExternalTokenizer,
        externalPropSource: { _ in htmlHighlighting },
        contextTracker: htmlElementContext
    ))
    return parser
}()

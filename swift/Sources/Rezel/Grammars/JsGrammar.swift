...
private nonisolated(unsafe) let jsTrackNewline = ContextTracker(
    start: false as Any,
...
private func makeJsExternalTokenizer(name: String, terms: [String: Int]) -> TokenizerProtocol {
    ...
    if name == "jsx" {
        ...
        return ExternalTokenizer({ input, stack in
            ...
            var back = 0
            while space.contains(input.next) { input.advance(); back += 1 }
            if jsIdentifierChar(input.next, true) {
                input.advance()
                back += 1
                while jsIdentifierChar(input.next, false) { input.advance(); back += 1 }
                while space.contains(input.next) { input.advance(); back += 1 }
...
private nonisolated(unsafe) let jsHighlighting = styleTags([

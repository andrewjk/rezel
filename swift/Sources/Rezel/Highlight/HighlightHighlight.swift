nonisolated(unsafe) private var _nextTagID: Int = 0
...
public class Tag: CustomStringConvertible {
    ...
    public static func define(_ name: String? = nil, parent: Tag? = nil) -> Tag {
        ...
        if let parent = parent {
            for t in parent.set { tagSet.append(t) }
        }
...
nonisolated(unsafe) private var _nextModifierID: Int = 0
...
public class Modifier {
    ...
    public static func get(_ base: Tag, mods: [Modifier]) -> Tag {
        ...
        for m in mods { m.instances.append(tag) }
...
private func hlSameArray(_ a: [Modifier], _ b: [Modifier]) -> Bool {
    ...
    for i in 0..<a.count { if a[i].id != b[i].id { return false } }
    return true
}
...
public class HighlightRule {
    ...
    public var opaque: Bool { mode == hlModeOpaque }
    public var inherit: Bool { mode == hlModeInherit }
    ...
    public func sort(_ other: HighlightRule?) -> HighlightRule {
        ...
        other.next = self.sort(other.next)
        return other
    }

    public var depth: Int { context?.count ?? 0 }

    nonisolated(unsafe) public static let empty = HighlightRule(tags: [], mode: hlModeNormal, context: nil)
}

nonisolated(unsafe) private let _ruleNodeProp = NodeProp<HighlightRule>(
...
public extension Highlighter {
    func scope(_ node: NodeType) -> Bool { false }
}
...
public func tagHighlighter(
    ...
    for style in tags {
        if let tag = style.tag as? Tag {
            ...
            for tag in tags { map[tag.id] = style.className }
        }
    }
...
nonisolated(unsafe) public let hlComment = Tag.define("comment")
nonisolated(unsafe) public let hlName = Tag.define("name")
nonisolated(unsafe) public let hlTypeName = Tag.define("typeName", parent: hlName)
nonisolated(unsafe) public let hlPropertyName = Tag.define("propertyName", parent: hlName)
nonisolated(unsafe) public let hlLiteral = Tag.define("literal")
nonisolated(unsafe) public let hlString = Tag.define("string", parent: hlLiteral)
nonisolated(unsafe) public let hlNumber = Tag.define("number", parent: hlLiteral)
nonisolated(unsafe) public let hlContent = Tag.define("content")
nonisolated(unsafe) public let hlHeading = Tag.define("heading", parent: hlContent)
nonisolated(unsafe) public let hlKeyword = Tag.define("keyword")
nonisolated(unsafe) public let hlOperator = Tag.define("operator")
nonisolated(unsafe) public let hlPunctuation = Tag.define("punctuation")
nonisolated(unsafe) public let hlBracket = Tag.define("bracket", parent: hlPunctuation)
nonisolated(unsafe) public let hlMeta = Tag.define("meta")

nonisolated(unsafe) public let hlTags: [String: Any] = {
    var t: [String: Any] = [:]
...
nonisolated(unsafe) public let classHighlighter: Highlighter = buildClassHighlighter()

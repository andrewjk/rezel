...
public class Tree: CustomStringConvertible {
    ...
    nonisolated(unsafe) public static let empty = Tree(type: NodeType.none, children: [], positions: [], length: 0)

    public func cursor(mode: IterMode = []) -> TreeCursor {
        return TreeCursor(node: topNode, mode: mode)
    }

    public func cursorAt(pos: Int, side: Int = 0, mode: IterMode = []) -> TreeCursor {

public class TreeCursor: SyntaxNodeRef {
    ...
    public init(node: SyntaxNode, mode: IterMode = []) {
        ...
        if let treeNode = node as? TreeNode {
            self._tree = treeNode
            self.type = treeNode.type
            self.from = treeNode.from
            self.to = treeNode.to
            self.buffer = nil
        } else if let bufNode = node as? BufferNode {
            self._tree = bufNode.context.parent
            self.buffer = bufNode.context
            self.type = bufNode.type
            self.from = bufNode.from
            self.to = bufNode.to
            self.index = bufNode.index
            var n: BufferNode? = bufNode._parent
            while let p = n {
                stack.insert(p.index, at: 0)
                n = p._parent
            }
            self.bufferNode = bufNode
        } else {
    ...
    public var node: SyntaxNode {
        ...
        let bn = BufferNode(context: buffer!, parent: result, index: self.index)
        bufferNode = bn
        return bn
    }
    ...
    public func firstChild() -> Bool { enterChild(1, pos: 0, side: .dontCare) }
    public func lastChild() -> Bool { enterChild(-1, pos: 0, side: .dontCare) }
    public func childAfter(_ pos: Int) -> Bool { enterChild(1, pos: pos, side: .after) }
    public func childBefore(_ pos: Int) -> Bool { enterChild(-1, pos: pos, side: .before) }
    ...
    public func nextSibling() -> Bool { sibling(1) }
    public func prevSibling() -> Bool { sibling(-1) }
    ...
    public func next(_ enter: Bool = true) -> Bool { move(1, enter: enter) }
    public func prev(_ enter: Bool = true) -> Bool { move(-1, enter: enter) }

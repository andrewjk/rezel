...
public typealias ParseWrapper = (inout AnyPartialParse, InputProtocol, [TreeFragment], [CommonRange]) -> AnyPartialParse

public class Parser {
	public init() {}

	public func createParse(input _: InputProtocol, fragments _: [TreeFragment], ranges _: [CommonRange]) -> any PartialParse {
		fatalError("Must be overridden")
	}

	public func startParse(input: Any, fragments: [TreeFragment] = [], ranges: [CommonRange]? = nil) -> any PartialParse {
		...
		let resolvedRanges: [CommonRange]
		if let r = ranges {
			if r.isEmpty {
				resolvedRanges = [CommonRange(from: 0, to: 0)]
			} else {
				resolvedRanges = r
			}
		} else {
			resolvedRanges = [CommonRange(from: 0, to: inputObj.length)]
		}
		return createParse(input: inputObj, fragments: fragments, ranges: resolvedRanges)
	}

	public func parse(input: Any, fragments: [TreeFragment] = [], ranges: [CommonRange]? = nil) -> Tree {
...
public struct AnyPartialParse: PartialParse {
	...
	public mutating func advance() -> Tree? {
		_base.advance()
	}
	...
	public mutating func stopAt(_ pos: Int) {
		_base.stopAt(pos)
	}

public class BufferNode: SyntaxNode {
    ...
    public var from: Int { context.start + Int(context.buffer.buffer[self.index + 1]) }
    public var to: Int { context.start + Int(context.buffer.buffer[self.index + 2]) }
    ...
    public init(context: BufferContext, parent: BufferNode?, index: Int) {
        ...
        self._parent = parent
        self.index = index
        self.type = context.buffer.set.types[Int(context.buffer.buffer[index])]
    }
    ...
    public var firstChild: SyntaxNode? { child(1, pos: 0, side: .dontCare) }
    public var lastChild: SyntaxNode? { child(-1, pos: 0, side: .dontCare) }
    public func childAfter(_ pos: Int) -> SyntaxNode? { child(1, pos: pos, side: .after) }
    public func childBefore(_ pos: Int) -> SyntaxNode? { child(-1, pos: pos, side: .before) }

    public func prop<T>(_ prop: NodeProp<T>) -> T? { type.prop(prop) }
    ...
    public var node: SyntaxNode { self }

...
) -> (AnyPartialParse, InputProtocol, [TreeFragment], [Range]) -> AnyPartialParse {
    return { parse, input, fragments, ranges in
        var mp = MixedParse(base: parse, nest: nest, input: input, fragments: fragments, ranges: ranges)
...
class InnerParse {
    ...
    init(parser: Parser, parse: any PartialParse, overlay: [Range]?, bracketed: Bool, target: Tree, from: Int) {
        self.parser = parser
        self.parse = parse
        self.overlay = overlay
        self.bracketed = bracketed
...
func checkRanges(_ ranges: [Range]) {
...
func checkCover(_ covered: [Range], from: Int, to: Int) -> Cover {
...
class MixedParse: PartialParse {
    ...
    init(base: AnyPartialParse, nest: @escaping (SyntaxNodeRef, InputProtocol) -> NestedParse?, input: InputProtocol, fragments: [TreeFragment], ranges: [Range]) {
        self.baseParse = base
        self.nest = nest
        self.input = input
    ...
    func advance() -> Tree? {
        if baseParse != nil {
            ...
            startInner()
            if stoppedAt != nil {
                for ip in inner { ip.parse.stopAt(stoppedAt!) }
            }
        }
        ...
        scan: while true {
            ...
            if stoppedAt != nil && cursor.from >= stoppedAt! {
                    ...
                    if let match = match {
                        for r in match.mount.overlay! {
                            ...
                            if from >= cursor.from && to <= cursor.to &&
                                ...
                                ov.ranges.append(Range(from: from, to: to))
                            }
                        }
                    }
                }
                ...
                if let overlayFn = nestResult.overlay as? (SyntaxNodeRef) -> Any {
                    ...
                    var nestRanges: [Range]
                    if let overlayRanges = nestResult.overlay as? [Range] {
                        nestRanges = punchRanges(outer: self.ranges, ranges: overlayRanges)
                    } else {
                        let r = cursor.from < cursor.to ? [Range(from: cursor.from, to: cursor.to)] : [Range]()
                        nestRanges = punchRanges(outer: self.ranges, ranges: r)
                    }
                    ...
                    if !nestRanges.isEmpty || nestResult.overlay == nil {
                        ...
                        let innerOverlay: [Range]?
                        if let overlayRanges = nestResult.overlay as? [Range] {
                            innerOverlay = overlayRanges.map { Range(from: $0.from - cursor.from, to: $0.to - cursor.from) }
                        } else {
                            innerOverlay = nil
                        }
                ...
                if let rangeVal = range as? Range {
                    if rangeVal.from < rangeVal.to {
                        if !ov.ranges.isEmpty && ov.ranges[ov.ranges.count - 1].to == rangeVal.from {
                            ov.ranges[ov.ranges.count - 1] = Range(from: ov.ranges[ov.ranges.count - 1].from, to: rangeVal.to)
                        } else {
                            ov.ranges.append(rangeVal)
                        }
                    }
                } else if let boolVal = range as? Bool, boolVal {
                    let rangeVal = Range(from: cursor.from, to: cursor.to)
                    if rangeVal.from < rangeVal.to {
                        if !ov.ranges.isEmpty && ov.ranges[ov.ranges.count - 1].to == rangeVal.from {
                            ov.ranges[ov.ranges.count - 1] = Range(from: ov.ranges[ov.ranges.count - 1].from, to: rangeVal.to)
                        } else {
                        ...
                        if ov.depth == 0 {
                            ...
                            if !ranges.isEmpty {
                                ...
                                let ip = InnerParse(
                                    ...
                                    overlay: ov.ranges.map { Range(from: $0.from - ov.start, to: $0.to - ov.start) },
                                    bracketed: ov.bracketed,
                                    target: ov.target,
                                    from: ranges[0].from
                                )
                                self.inner.insert(ip, at: ov.index)
                            }
                            overlay = ov.prev
                        }
                    }
...
func checkCoverValue(_ covered: [Range], from: Int, to: Int) -> Cover? {
...
func punchRanges(outer: [Range], ranges: [Range]) -> [Range] {
    var current: [Range]? = nil
    var j = 0
    for i in 1..<outer.count {
        ...
        while j < (current ?? ranges).count {
            ...
            if r.from < gapFrom {
                current![j] = Range(from: r.from, to: gapFrom)
                if r.to > gapTo {
                    current!.insert(Range(from: gapTo, to: r.to), at: j + 1)
                }
            } else if r.to > gapTo {
                current![j] = Range(from: gapTo, to: r.to)
                j -= 1
            } else {
                current!.remove(at: j)
                j -= 1
...
func findCoverChanges(
    a: [Range], b: [Range], from: Int, to: Int
) -> [Range] {
    var iA = 0, iB = 0
    ...
    while true {
        ...
        if inA != inB {
            ...
            if start < end { result.append(Range(from: start, to: end)) }
        }
        pos = min(nextA, nextB)
...
func enterFragments(_ mounts: [ReusableMount], ranges: [Range]) -> [TreeFragment] {
    ...
    for mount in mounts {
        ...
        if let overlay = mount.mount.overlay {
            let overlayRanges = overlay.map { Range(from: $0.from + mount.pos, to: $0.to + mount.pos) }
...
class StructureCursor {
    ...
    init(root: Tree, offset: Int) {
        self.offset = offset
        self.cursor = root.cursor(mode: [.includeAnonymous, .ignoreMounts])
    }
...
class FragmentCursor {
    ...
    func hasNode(_ node: TreeCursor) -> Bool {
        while curFrag != nil && node.from >= curTo { nextFrag() }

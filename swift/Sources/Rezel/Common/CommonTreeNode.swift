public class TreeNode: SyntaxNode {
    ...
    public var type: NodeType { _tree.type }
    public var name: String { _tree.type.name }
    public var to: Int { from + _tree.length }
    public var tree: Tree? { _tree }
    ...
    public func enter(_ pos: Int, side: Int, mode: IterMode? = nil) -> SyntaxNode? {
        ...
        if !mode.contains(.ignoreOverlays), let mounted = MountedTree.get(_tree), mounted.overlay != nil {
            ...
            for range in mounted.overlay! {
                ...
                if ok1 && ok2 {
                    return TreeNode(tree: mounted.tree, from: mounted.overlay![0].from + self.from, index: -1, parent: self)
                }
            }
        }
    ...
    func nextSignificantParent() -> TreeNode {
        ...
        while val.type.isAnonymous, let p = val._parent { val = p }
        return val
    }

...
public class Pos: CustomStringConvertible {
	...
	public func trail(_ maxLen: Int = 60) -> String {
		...
		var p: Pos? = self
		while let current = p {
			for i in stride(from: current.pos - 1, through: 0, by: -1) {
				result.append(current.rule.parts[i])
			}
			p = current.via
		}
		...
		if value.count > maxLen {
			...
			value = "…" + value[start...]
			if value.range(of: " ", options: .literal, range: value.startIndex ..< (value.index(after: value.startIndex))) != nil {
...
func cmpStr(_ a: String, _ b: String) -> Int {
	a < b ? -1 : a > b ? 1 : 0
}
...
public class Shift: CustomStringConvertible, Hashable {
	...
	public init(term: Term, target: AutState) {
		self.term = term; self.target = target
	}
...
public class Reduce: CustomStringConvertible, Hashable {
	...
	public init(term: Term, rule: Rule) {
		self.term = term; self.rule = rule
	}
	...
	public func matches(_ other: ActionItem, _: [Int]) -> Bool {
		guard let o = other as? Reduce else { return false }
		return o.rule.sameReduce(rule)
	}

	public var description: String {
		"\(rule.name.name)(\(rule.parts.count))"
	}
...
class AutConflict {
	...
	init(error: String, rules: [Term]) {
		self.error = error; self.rules = rules
	}
}
...
public class AutState: CustomStringConvertible {
	...
	public init(id: Int, set: [Pos], flags: Int = 0, skip: Term, hash: Int? = nil, startRule: Term? = nil) {
		...
		hashValue = hash ?? hashPositions(set); self.startRule = startRule
	}
	...
	public func hasSet(_ set: [Pos]) -> Bool {
		eqSetPos(self.set, set)
	}

	private var _actionsByTerm: [Int: [ActionItem]]?
...
func eqSetActions(_ a: [ActionItem], _ b: [ActionItem]) -> Bool {
	if a.count != b.count { return false }
	for i in 0 ..< a.count {
		if !actionEq(a[i], b[i]) { return false }
	}
	return true
}
			...
			while i < state.gotoActions.count {
				...
				if !hasShift {
					let hasNilShift = start.contains(where: { $0 == nil }) && state.actions.contains(where: { a in
						guard a is Shift else { return false }
						return true
					})
					if !hasNilShift {
...
class AutGroup {
	...
	init(origin: Int, member: Int) {
		self.origin = origin; members = [member]
	}
}

...
class Parts {
	...
	func concat(_ other: Parts) -> Parts {
		...
		if conflicts != nil || other.conflicts != nil {
			var c = conflicts ?? ensureConflicts()
		...
		return Parts(terms: terms + other.terms, conflicts: mergedConflicts)
	}
...
class BuildingRule {
	let name: String
	let start: GeneratorState
	let to: GeneratorState
	let args: [Expression]

	init(name: String, start: GeneratorState, to: GeneratorState, args: [Expression]) {
		self.name = name
		self.start = start
		self.to = to
		self.args = args
...
class TokenSet {
	var startState: GeneratorState = .init()
	var built: [BuiltRule] = []
	...
	func getToken(_ expr: NameExpression) -> Term? {
		for builtRule in built where builtRule.matches(expr) {
			return builtRule.term
		}
		...
		buildRule(rule, expr, startState, GeneratorState(accepting: [term]))
	...
	func buildRule(
		...
		if let buildingRule = building.first(where: { $0.name == name && exprsEq(expr.args, $0.args) }) {
			...
			while building[lastIndex].name != name {
				lastIndex -= 1
			}
			b.raise(
	...
	func build(_ expr: Expression, _ from: GeneratorState, _ to: GeneratorState, _ args: [TokenArg]) {
		if let nameExpr = expr as? NameExpression {
			...
			for choice in choiceExpr.exprs {
				build(choice, from, to, args)
			}
			...
			for i in 0 ..< seqExpr.exprs.count {
				let next = i == seqExpr.exprs.count - 1 ? to : GeneratorState()
			...
			for (a, bVal) in ranges {
				rangeEdges(from, to, a, bVal)
			}
			...
			for i in 0 ..< utf16.count {
				let ch = Int(utf16[i])
				let next = i == utf16.count - 1 ? to : GeneratorState()
				current.edge(ch, ch + 1, next)
				current = next
			}
		} else if expr is AnyExpression {
			let mid = GeneratorState()
...
class MainTokenSet: TokenSet {
	...
	func getLiteral(_ expr: LiteralExpression) -> Term {
		...
		for builtRule in built where builtRule.id == id {
			return builtRule.term
		}
		...
		build(expr, startState, GeneratorState(accepting: [term]), [])
	...
	func takeConflicts() {
		func resolve(_ expr: Expression) -> Term? {
			if let nameExpr = expr as? NameExpression {
				for builtRule in built where builtRule.matches(nameExpr) {
					return builtRule.term
				}
			} else if let litExpr = expr as? LiteralExpression {
	...
	func buildLocalGroup(
		...
		for state in states {
			...
			for action in state.actions {
				...
				while let o = orig, let spec = o.spec {
					orig = b.tokenOrigins[spec.name]
				}
		...
		for val in precTable {
			fullData.append(UInt16(truncatingIfNeeded: val))
		}
...
class ExternalTokenSet: TokenizerSpecProtocol {
	...
	init(b: Builder, ast: ExternalTokenDeclaration) {
		self.b = b
		self.ast = ast
		tokens = gatherExtTokens(b, ast.tokens)
...
class Builder {
	...
	init(text: String, options: BuildOptions) throws {
		...
		input = parsedInput
		ast = try logTime("Parse") { try parsedInput.parse() }
		...
		logTime("Build rules") {
			...
			for rule in ast.rules {
				astRules.append((skip: mainSkip, rule: rule))
			}
			for rule in ast.topRules {
				topRules.append((rule: rule, skip: mainSkip))
			}
			for scoped in ast.scopedSkip {
				...
				for rule in scoped.rules {
					astRules.append((skip: skip, rule: rule))
				}
				for rule in scoped.topRules {
					topRules.append((rule: rule, skip: skip))
				}
			}

			for astRule in astRules {
				unique(astRule.rule.id)
			}
		...
		for entry in definedGroups {
			defineGroup(entry.name, entry.group, entry.rule)
		}
		checkGroups()
	}
	...
	func prepareParser() throws -> (
		...
		for (_, term) in namedTerms {
			termTable[namedTerms.first(where: { $0.value === term })?.key ?? ""] = term.id
		}
		for (name, term) in namedTerms {
			termTable[name] = term.id
		}
		...
		if !dynamicRulePrecedences.isEmpty {
			dynamicPrecedences = [:]
			for entry in dynamicRulePrecedences {
				dynamicPrecedences![entry.rule.id] = entry.prec
			}
		}
	...
	func nodeInfo(
		...
		if let defaultProps = defaultProps, hasProps(defaultProps) {
			for (prop, value) in defaultProps {
				if result[prop] == nil { result[prop] = value }
			}
		}
...
func rangeEdges(_ from: GeneratorState, _ to: GeneratorState, _ low: Int, _ hi: Int) {
	var low = low, hi = hi
	if low < ASTRAL_CODE {
...
func mergeRules(_ rules: [Rule]) -> [Rule] {
	...
	while i < rules.count {
		...
		i += 1
		while i < rules.count, rules[i].name === name {
			i += 1
		}
		...
		while j < rules.count {
			...
			j += 1
			while j < rules.count && rules[j].name === otherName {
				j += 1
			}

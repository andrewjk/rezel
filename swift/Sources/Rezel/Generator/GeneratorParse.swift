...
public class GenInput {
	...
	private var pos: Int {
		string.distance(from: string.startIndex, to: stringIndex)
	}
	...
	public func unexpected() -> Never {
		raise("Unexpected token '\(string[string.index(string.startIndex, offsetBy: start) ..< string.index(string.startIndex, offsetBy: end)])'", start)
	}
	...
	public func parse() throws -> GrammarDeclaration {
		try parseGrammar(self)
	}
}
...
private func parseGrammar(_ input: GenInput) throws -> GrammarDeclaration {
	...
	while input.type != "eof" {
		...
		if input.eat("at", "top") {
			...
			try tops.append(parseRule(input, named: parseIdent(input)))
			sawTop = true
			...
			if input.type == "{" {
				...
				while !input.eat("}") {
					if input.eat("at", "top") {
						try skipTopRules.append(parseRule(input, named: parseIdent(input)))
						sawTop = true
					} else {
						try skipRules.append(parseRule(input))
					}
				}
			...
			try rules.append(parseRule(input))
		}
	}
...
private func parseExprChoice(_ input: GenInput) -> Expression {
	...
	repeat {
		exprs.append(parseExprSequence(input))
	} while input.eat("|")

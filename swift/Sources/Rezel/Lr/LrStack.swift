...
public class Stack: CustomStringConvertible {
    ...
    public func reduce(_ action: Int) {
        ...
        if depth == 0 {
            ...
            pushState(parser.getGoto(state: self.state, term: type, loose: true), reducePos)
        ...
        var base = stack.count - (depth - 1) * 3 - ((action & Action.StayFlag) != 0 ? 6 : 0)
        ...
        let count = self.bufferBase + self.buffer.count - bufferBaseVal

        if type < parser.minRepeatTerm || (action & Action.RepeatFlag) != 0 {
            let pos = parser.stateFlag(self.state, flag: StateFlag.Skipped) ? self.pos : reducePos
            storeNode(term: type, start: start, end: pos, size: count + 4, mustSink: true)
        }
        if (action & Action.StayFlag) != 0 {
            self.state = stack[base]
        } else {
            let baseStateID = stack[base - 3]
            self.state = parser.getGoto(state: baseStateID, term: type, loose: true)
        }
        while stack.count > base { stack.removeLast() }
    ...
    public func storeNode(term: Int, start: Int, end: Int, size: Int = 4, mustSink: Bool = false) {
        ...
        if !mustSink || pos == end {
            ...
            while buffer.count < index + 4 { buffer.append(0) }
            buffer[index] = term
    ...
    public func split() -> Stack {
        ...
        while let p = parent, base == p.bufferBase { parent = p.parent }
        return Stack(
            p: self.p,
            stack: Array(self.stack),
            state: self.state,
            reducePos: self.reducePos,
            pos: self.pos,
            score: self.score,
            buffer: buf,
            bufferBase: base,
    ...
    public func findForcedReduction() -> Int? {
        ...
        return explore(self.state, depth: 0)
    }
    ...
    public var parser: LRParser { p.parser }
...
public class StackBufferCursor: BufferCursorProtocol {
    ...
    public var id: Int { buffer[index - 4] }
    public var start: Int { buffer[index - 3] }
    public var end: Int { buffer[index - 2] }
    public var size: Int { buffer[index - 1] }

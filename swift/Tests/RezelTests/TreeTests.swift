...
private nonisolated(unsafe) let testEnv = makeTestTypes()
private nonisolated(unsafe) let testTypes = testEnv.types
private nonisolated(unsafe) let testNodeSet = testEnv.nodeSet
...
private nonisolated(unsafe) var _recur: Tree?
...
private nonisolated(unsafe) var _simple: Tree?
...
private nonisolated(unsafe) let anonTree = Tree(
...
struct SyntaxNodeTests {
	...
	func resolveLarge() throws {
		var c: SyntaxNode? = recur().resolve(pos: 10, side: 1)
		var depth = 1
		while c != nil {
			c = try #require(c?.parent); depth += 1
		}
		#expect(depth == 9)
	}
	...
	func accessTree() throws {
		let tree = mk("aaa[bbbbb(bb)bbbbbbb]aaa")
		var node = try #require(tree.topNode.firstChild)
		while node.name != "Br" {
			node = try #require(node.nextSibling)
		}
		#expect(node.tree != nil)
		#expect(node.tree?.type.name == "Br")
		node = try #require(node.firstChild)
		while node.name != "Pa" {
			node = try #require(node.nextSibling)
		}
		#expect(node.tree == nil)

...
private func shared(_ a: Tree, _ b: Tree) -> Int {
    ...
    func register(_ obj: Any) {
        if let tree = obj as? Tree {
            ...
            for ch in target.children { register(ch) }
    ...
    func scan(_ obj: Any) {
        if let tree = obj as? Tree {
            if inA.contains(ObjectIdentifier(tree)) {
                ...
                let mounted = MountedTree.get(tree)
                let target = (mounted != nil && mounted!.overlay == nil) ? mounted!.tree : tree
                for ch in target.children { scan(ch) }
            }
        } else if let buf = obj as? TreeBuffer {
...
struct ParsingTests {
    ...
    func incrementalParse() throws {
        ...
        let pos = doc.distance(from: doc.startIndex, to: doc.range(of: "false")!.lowerBound)
    ...
    func invalidInput() {
        ...
        #expect(ast.description.contains("T"))
    }
...
@Suite("sequences", .serialized)
struct SequencesTests {
    nonisolated(unsafe) private static let p1: LRParser = p("""
    @top T { (X | Y)+ }
    @skip { C }
    C { "c" }
    X { "x" }
...
@Suite("multiple tops", .serialized)
...
@Suite("mixed languages", .serialized)
struct MixedLanguagesTests {
    nonisolated(unsafe) private static let blob: LRParser = p("""
    @top Blob { ch* } @tokens { ch { _ } }
    """)

    nonisolated(unsafe) private static let templateParser: LRParser = p("""
    ...
    private func wrapMixed(_ mixed: @escaping (AnyPartialParse, InputProtocol, [TreeFragment], [Range]) -> AnyPartialParse) -> ParseWrapper {
        return { parse, input, fragments, ranges in
            return mixed(parse, input, fragments, ranges)
        }
    }
    ...
    func mountEmptyNodes() throws {
        ...
        let mix = Self.templateParser.configure(
            wrap: wrapMixed(parseMixed { node, _ in
                return node.name == "BlockContent" ? NestedParse(parser: inner) : nil
            })
        )
    ...
    func fragmentOffsets() throws {
            ...
            wrap: wrapMixed(parseMixed { node, _ in
                if node.name == "Templ" {
                    return NestedParse(
                        parser: inner,
                        overlay: { (n: SyntaxNodeRef) -> Any in
                            return n.name == "String" ? Range(from: n.from + 1, to: n.to - 1) : false
                        }
                    )
                }
                return nil
        ...
        #expect(shared(tree, tree2) > 20)
        #expect(shared(tree.resolveInner(pos: 49).tree!, tree2.resolveInner(pos: 50).tree!) > 20)
            ...
            outer.configure(
                wrap: wrapMixed(parseMixed { node, _ in
                    ...
                    func scan(_ node: SyntaxNode) {
                        if node.name == "String" {
                            ranges.append(Range(from: node.from + 1, to: node.to - 1))
                        } else {
    ...
    func reparseCutOffInnerParses() throws {
        ...
        """).configure(
            wrap: wrapMixed(parseMixed { node, _ in
                return node.name == "SectionContent" ? NestedParse(parser: inner) : nil
            }),
            bufferLength: 2
        )
        let input = "{" + "<" + String(repeating: "x", count: 100) + ">}" + "{<xxxx>}"
        var parse = parser.startParse(input: input)
        while parse.parsedPos < 50 { _ = parse.advance() }
        parse.stopAt(parse.parsedPos)
        var tree1: Tree? = nil
        while true {
            if let t = parse.advance() { tree1 = t; break }
        }
        try testTree(tree1!, "Doc(Section(Phrase(⚠)),Section(Phrase(⚠)))")
        let tree2 = parser.parse(input: input, fragments: TreeFragment.addTree(tree1!))

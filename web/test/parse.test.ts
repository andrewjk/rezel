...
describe("parsing", () => {
	...
	it("can parse incrementally", () => {
		...
		ist(ast.length, 700);
		let pos = doc.indexOf("false"),
			doc2 = doc.slice(0, pos) + "x" + doc.slice(pos + 5);
		let ast2 = p1()
			.configure({ bufferLength: 2 })
			.parse(doc2, fragments(ast, [pos, pos + 5, pos, pos + 1]));
	...
	function testIter(bufferLength: number, partial: boolean) {
		...
		ast.iterate({
			...
			enter(n) {
				output.push(n.name, n.from);
			},
			leave(n) {
				output.push("/" + n.name, n.to);
			},
		});
...
@tokens {
  ...
  commentContent { ![()]+ }
		...
		let ast2 = comments
			...
			.parse(doc.slice(1), fragments(ast, [0, 1, 0, 0]));
  ...
  Char { _ }
		...
		let tree2 = comments.parse(
			...
			TreeFragment.applyChanges(TreeFragment.addTree(tree1), [
				{ fromA: 1000, toA: 1000, fromB: 1000, toB: 1001 },
			]),
		);
...
describe("sequences", () => {
	...
	function breadth(tree: any): number {
		return tree instanceof Tree
			? tree.children.reduce((b, c) => Math.max(b, breadth(c)), tree.children.length)
			: 0;
	}
...
@tokens {
  ...
  b { "b" }
	...
	it("caches parts of sequences", () => {
		let doc = "x".repeat(1000),
			p = p1().configure({ bufferLength: 10 });
		let ast = p.parse(doc);
...
describe("mixed languages", () => {
    ...
    `).configure({
			wrap: parseMixed((node, input) => {
				if (node.name == "Content") {
					...
					if (input.read(open.from, open.to) == "<script>") return { parser: inner };
				}
				return null;
			}),
		});
		testTree(outer.parse("<foo>bar</foo>"), "T(Tag(Open,Content,Close))");
		testTree(outer.parse("<script>hello</script>"), "T(Tag(Open,Script,Close))");
	});
    ...
    `).configure({
		...
		let base = "hello {bbbb} ";
		let doc = base.repeat(500) + "{" + "b".repeat(1000) + "} " + base.repeat(500),
			off = base.length * 500 + 500;
		...
		let ast2 = outer.parse(
			...
			TreeFragment.applyChanges(TreeFragment.addTree(ast1), [
				{ fromA: off, toA: off, fromB: off, toB: off + 3 },
			]),
		);
	...
	it("can resolve a stack", () => {
      ...
      @tokens { Text { ![()]+ } }`);
		let mix = templateParser().configure({
			wrap: parseMixed((node) =>
				node.type.isTop ? { parser: parens, overlay: (n) => n.name == "Content" } : null,
			),
		});
	...
	it("properly handles fragment offsets", () => {
		let inner = buildParser(`@top Text { (Word | " ")* } @tokens { Word { ![ ]+ } }`).configure({
			bufferLength: 2,
		});
		let outer = buildParser(`
      @top Doc { expr* }
    ...
    `).configure({
			...
			wrap: parseMixed((node) => {
				return node.name == "Templ"
					? {
							parser: inner,
							overlay: (node) =>
								node.name == "String" ? { from: node.from + 1, to: node.to - 1 } : false,
						}
					: null;
			}),
		});
		...
		let tree2 = outer.parse(
			"88" + doc.slice(0, 30) + doc.slice(31),
	...
	it("re-parses cut-off inner parses even if the outer tree was finished", () => {
		let inner = buildParser(`@top Phrase { "<" ch* ">" } @tokens { ch { ![>] } }`).configure({
			bufferLength: 2,
		});
		let parser = buildParser(`
      @top Doc { Section* }

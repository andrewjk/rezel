...
function externalTokenizer(name: string, terms: { [name: string]: number }) {
	if (name == "ext1")
		return new ExternalTokenizer((input: InputStream) => {
			...
			} else if (input.next == "}".charCodeAt(0)) {
				input.advance();
...
function externalSpecializer(name: string, terms: { [name: string]: number }) {
	if (name == "spec1")
		return (value: string) => (value == "one" ? terms.one : value == "two" ? terms.two : -1);
...
describe("Cases", () => {
	for (let file of fs.readdirSync(caseDir)) {
		...
		if (!match) continue;
		let name = match[1],
			fileName = path.join(caseDir, file);
		...
		if (expectedErr)
			it(`${name} fails`, () => {
				ist.throws(
					force,
					(e: Error) => e.message.toLowerCase().indexOf(expectedErr![1].trim().toLowerCase()) >= 0,
				);
			});
		if (!noCases)
			for (let { name: n, run } of fileTests(content, file)) it(name + "/" + n, () => run(force()));
	}
});

...
import { Tree, NodeSet, NodeType, SyntaxNode, NodeProp, IterMode } from "../src/common";

let types = "T a b c Pa Br".split(" ").map((s, i) =>
	NodeType.define({
		id: i,
		name: s,
...
function id(n: string) {
	return types.find((x) => x.name == n)!.id;
}
...
const anonTree = new Tree(
	nodeSet.types[0],
	[
		new Tree(
			NodeType.none,
			[new Tree(nodeSet.types[1], [], [], 1), new Tree(nodeSet.types[2], [], [], 1)],
			[0, 1],
			2,
		),
	],
...
describe("SyntaxNode", () => {
	...
	it("can resolve in a large tree", () => {
		let c: SyntaxNode | null = recur().resolve(10, 1),
			depth = 1;
...
describe("TreeCursor", () => {
	...
	it("internal iteration can be limited to a range", () => {
		...
		simple().iterate({
			enter(t) {
				seen.push(t.name);
				return t.name == "Br" ? false : undefined;
			},
			from: 3,
			to: 14,
		});
	...
	it("can move to a given position", () => {
		let tree = recur(),
			start = tree.length >> 1,
			cursor = tree.cursorAt(start, 1);
		do {
			ist(cursor.from, start, ">=");
		} while (cursor.next());
	});

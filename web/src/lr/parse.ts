		...
		if (this.fragments) {
			...
			for (let cached = this.fragments.nodeAt(start); cached; ) {
				...
				if (
					...
					if (verbose)
						console.log(
							base + this.stackID(stack) + ` (via reuse of ${parser.getName(cached.type.id)})`,
						);
					return true;
				}
		...
		if (defaultReduce > 0) {
			...
			if (verbose)
				console.log(
					base +
						this.stackID(stack) +
						` (via always-reduce ${parser.getName(defaultReduce & Action.ValueMask)})`,
				);
			return true;
		}
			...
			if (verbose)
				console.log(
					base +
						...
						` (via ${
							(action & Action.ReduceFlag) == 0
								? "shift"
								: `reduce of ${parser.getName(action & Action.ValueMask)}`
	...
	private runRecovery(stacks: Stack[], tokens: number[], newStacks: Stack[]) {
		...
		for (let i = 0; i < stacks.length; i++) {
			...
			if (this.stream.end > stack.pos) {
				...
				if (verbose)
					console.log(
						base + this.stackID(stack) + ` (via recover-delete ${this.parser.getName(token)})`,
					);
				pushStackDedup(stack, newStacks);
...
export class LRParser extends Parser {
		...
		if (spec.nodeProps)
			for (let propSpec of spec.nodeProps) {
				...
				if (typeof prop == "string")
					prop = (NodeProp as unknown as { [name: string]: NodeProp<any> })[prop];
	...
	hasAction(state: number, terminal: number) {
		...
		for (let set = 0; set < 2; set++) {
			for (
				let i = this.stateSlot(state, set ? ParseState.Skip : ParseState.Actions), next;
				;
				i += 3
			) {

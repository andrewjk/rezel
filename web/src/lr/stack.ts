...
export class Stack {
	...
	reduce(action: number): void {
		...
		let lookaheadRecord =
			this.reducePos < this.pos - Lookahead.Margin && this.setLookAhead(this.pos);
	...
	storeNode(term: number, start: number, end: number, size = 4, mustSink = false): void {
		if (
			...
			if (top > 0 && this.buffer[top - 4] == Term.Err && this.buffer[top - 1] > -1) {
				if (start == end) return;
	...
	shift(action: number, type: number, start: number, end: number): void {
		if (action & Action.GotoFlag) {
			...
			let nextState = action,
				{ parser } = this.p;
			this.pos = end;
			let skipped = parser.stateFlag(nextState, StateFlag.Skipped);
	...
	split(): Stack {
		...
		if (off && parent.buffer[off - 4] == Term.Err) off -= 4;
	...
	recoverByInsert(next: number): Stack[] {
		...
		if (
			nextStates.length > Recover.MaxNext << 1 ||
			this.stack.length >= Recover.DampenInsertStackDepth
		) {
			let best = [];
	...
	get deadEnd(): boolean {
		...
		let { parser } = this.p;
		return (
			parser.data[parser.stateSlot(this.state, ParseState.Actions)] == Seq.End &&
...
class SimulatedStack {
	...
	reduce(action: number) {
		let term = action & Action.ValueMask,
			depth = action >> Action.ReduceDepthShift;
		if (depth == 0) {

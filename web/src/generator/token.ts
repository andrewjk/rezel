...
function minimize(states: State[], start: State) {
	...
	for (;;) {
		let split = false,
			newPartition: Partition = Object.create(null);
		for (let state of states) {
	...
	for (let state of states) {
		for (let i = 0; i < state.edges.length; i++) {
			let edge = state.edges[i],
				target = partition[edge.target.id][0];
...
export class State {
	...
	compile() {
		...
		function explore(states: State[]) {
			let newState = (labeled[ids(states)] = new State(
				states.reduce((a: readonly Term[], s: State) => union(a, s.accepting), []) as Term[],
				localID++,
			));
			let out: Edge[] = [];
			for (let state of states)
	...
	closure() {
		...
		function explore(state: State): void {
			...
			if (
				state.edges.some((e) => e.from >= 0) ||
				(state.accepting.length > 0 &&
					!state.edges.some((e) => sameSet(state.accepting, e.target.accepting)))
			)
				result.push(state);
	...
	findConflicts(occurTogether: (a: Term, b: Term) => boolean): Conflict[] {
		...
		function add(a: Term, b: Term, soft: number, aEdges: Edge[], bEdges?: Edge[]) {
			...
			if (!found)
				conflicts.push(
					new Conflict(a, b, soft, exampleFromEdges(aEdges), bEdges && exampleFromEdges(bEdges)),
				);
						...
						for (let orig of state.accepting)
							if (term != orig)
								add(
									term,
									orig,
									hasCycle || cycleTerms.includes(orig) || !occurTogether(term, orig) ? 0 : 1,
									edges,
									edges.concat(es),
								);
					}
	...
	for (let i = 1; i < separate.length; i++) {
		...
		for (let edge of edges)
			if (edge.to > from && edge.from < to) {
				for (let target of edge.target.closure()) if (!found.includes(target)) found.push(target);
			}

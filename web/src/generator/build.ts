...
export type BuildOptions = {
	...
	contextTracker?:
		| ContextTracker<any>
		| ((terms: { [name: string]: number }) => ContextTracker<any>);
};

type SkipInfo = {
	skip: readonly Term[];
	rule: Term | null;
	startTokens: readonly Term[];
	id: number;
};
...
class Builder {
	...
	tokenOrigins: {
		[name: string]: {
			spec?: Term;
			external?: ExternalTokenSet | ExternalSpecializer;
			group?: LocalTokenSet;
		};
	} = Object.create(null);
	rules: Rule[] = [];
	...
	knownProps: {
		[name: string]: { prop: NodeProp<any>; source: { name: string; from: string | null } };
	} = Object.create(null);
	...
	constructor(text: string, options: BuildOptions) {
		...
		for (let prop of this.ast.externalProps) {
			this.knownProps[prop.id.name] = {
				prop: this.options.externalProp
					? this.options.externalProp(prop.id.name)
					: new NodeProp<string>(),
		...
		this.externalSpecializers = this.ast.externalSpecializers.map(
			(decl) => new ExternalSpecializer(this, decl),
		);
		...
		time("Build rules", () => {
			...
			let scopedSkip: Term[] = [],
				topRules: { rule: RuleDeclaration; skip: Term }[] = [];
			...
			for (let scoped of this.ast.scopedSkip) {
				let skip = noSkip,
					found = this.ast.scopedSkip.findIndex(
						(sc, i) => i < scopedSkip.length && exprEq(sc.expr, scoped.expr),
					);
			...
			if (mainSkip != noSkip) this.defineRule(mainSkip, this.normalizeExpr(this.ast.mainSkip!));
	...
	prepareParser() {
		let rules = time("Simplify rules", () =>
			simplifyRules(this.rules, [...this.skipRules, ...this.terms.tops]),
		);
		...
		let skipInfo: readonly SkipInfo[] = this.skipRules.map((name, id) => {
			...
			for (let rule of name.rules) {
				...
				if (
					start.terminal &&
					rule.parts.length == 1 &&
					!rules.some((r) => r != rule && r.parts[0] == start)
				)
					skip.push(start);
		...
		let fullTable = time("Build full automaton", () =>
			buildFullAutomaton(this.terms, startTerms, first),
		);
		let localTokens = this.localTokens.map((grp, i) => grp.buildLocalGroup(fullTable, skipInfo, i));
		let { tokenGroups, tokenPrec, tokenData } = time("Build token groups", () =>
			this.tokens.buildTokenGroups(fullTable, skipInfo, localTokens.length),
		);
		...
		let specialized: (ExternalSpecializer | { token: Term; table: { [value: string]: number } })[] =
			[];
		for (let ext of this.externalSpecializers) specialized.push(ext);
		for (let name in this.specialized)
			specialized.push({
				token: this.terms.names[name],
				table: buildSpecializeTable(this.specialized[name]),
			});
		...
		let tokenizers = (
			(tokenGroups as (TokenGroup | ExternalTokenSet)[])
		...
		let skipData = skipInfo.map((info) => {
			let actions: number[] = [];
			for (let term of info.skip) actions.push(term.id, 0, Action.StayFlag >> 16);
			if (info.rule) {
		...
		let states = time("Finish states", () => {
			...
			let finishCx = new FinishStateContext(
				tokenizers,
				data,
				states,
				skipData,
		...
		for (let i = 0; i < this.dialects.length; i++)
			dialects[this.dialects[i]] = data.storeArray(
				(this.tokens.byDialect[i] || none).map((t) => t.id).concat(Seq.End),
			);
	...
	getParserFile() {
		...
		let gen = "// This file was generated by lezer-generator. You probably shouldn't edit it.\n",
			head = gen;
		let imports: { [source: string]: string[] } = {},
			imported: { [spec: string]: string } = Object.create(null);
		...
		let context = this.ast.context
			? importName(this.ast.context.id.name, this.ast.context.source)
			: null;
		...
		function specializationTableString(table: { [name: string]: number }) {
			return (
				...
				Object.keys(table)
					.map(
						(key) =>
							`${/^(\d+|[a-zA-Z_]\w*)$/.test(key) ? key : JSON.stringify(key)}:${table[key]}`,
					)
					.join(", ") +
				"}"
			);
		...
		let specialized = rawSpecialized.map((v) => {
			if (v instanceof ExternalSpecializer) {
				...
				}, external: ${name}${v.ast.type == "extend" ? ", extend: true" : ""}}`;
			} else {
		...
		for (let source in imports) {
			if (mod == "cjs") head += `const {${imports[source].join(", ")}} = require(${source})\n`;
		...
		function serializePropValue(value: any) {
			return typeof value != "string" || /^(true|false|\d+(\.\d+)?|\.\d+)$/.test(value)
				? value
  ...
  dialects: {${dialects.join(", ")}}`
		...
		dynamicPrecedences
			? `,
  dynamicPrecedences: ${JSON.stringify(dynamicPrecedences)}`
			: ""
	}${
		specialized.length
			? `,
  ...
  termNames: ${JSON.stringify(termNames)}`
	...
	gatherNodeProps(nodeTypes: readonly Term[]) {
		...
		for (let type of nodeTypes) {
			...
			for (let prop in type.props) {
				...
				if (
					known.source.from == null &&
					(known.source.name == "repeated" || known.source.name == "error")
				)
					continue;
	...
	computeForceReductions(states: readonly LRState[], skipInfo: readonly SkipInfo[]) {
		...
		// entries.
		let gotoEdges: { [term: number]: { parents: number[]; target: number }[] } =
			Object.create(null);
		for (let state of states) {
			...
			candidates[state.id] = state.set
				.filter((pos) => pos.pos > 0 && !pos.rule.name.top)
		...
		function createsCycle(
			...
			return edges.some((val) => {
				let parentIntersection = parents
					? parents.filter((id) => val.parents.includes(id))
					: val.parents;
	...
	substituteArgsInProps(
		...
		let substituteInValue = (value: readonly PropPart[]) => {
			...
			for (let i = 0; i < value.length; i++) {
				...
				else this.raise(`Trying to interpolate expression '${expr}' into a prop`, part.start);
			}
			return result;
		};
		...
		for (let i = 0; i < props.length; i++) {
			let prop = props[i],
				value = substituteInValue(prop.value);
			if (value != prop.value) {
	...
	conflictsFor(markers: readonly ConflictMarker[]) {
		let here = Conflicts.none,
			atEnd = Conflicts.none;
		for (let marker of markers) {
			if (marker.type == "ambig") {
				...
				let prec = precs.items[index],
					value = precs.items.length - index;
				if (prec.type == "cut") {
					here = here.join(new Conflicts(0, none, value));
				} else {
					here = here.join(new Conflicts(value << 2));
					atEnd = atEnd.join(
						new Conflicts((value << 2) + (prec.type == "left" ? 1 : prec.type == "right" ? -1 : 0)),
					);
				}
			}
		}
	...
	resolve(expr: NameExpression): Parts[] {
		...
		if (!known) return this.raise(`Reference to undefined rule '${expr.id.name}'`, expr.start);
	...
	normalizeSequence(expr: SequenceExpression) {
		...
		function complete(start: Parts, from: number, endConflicts: Conflicts): Parts[] {
			...
			for (let choice of result[from]) {
				for (let full of complete(
					start.concat(choice).withConflicts(start.terms.length, here),
					from + 1,
					endConflicts.join(atEnd),
				))
					choices.push(full);
			}
			return choices;
	...
	normalizeExpr(expr: Expression): Parts[] {
		if (expr instanceof RepeatExpression && expr.kind == "?") {
			...
			return [
				p(this.buildRule(expr.rule, none, this.currentSkip[this.currentSkip.length - 1], true)),
			];
		} else {
			return this.raise(
				`This type of expression ('${expr}') may not occur in non-token rules`,
				expr.start,
			);
		}
	}
	...
	buildRule(rule: RuleDeclaration, args: readonly Expression[], skip: Term, inline = false): Term {
		...
		let name = this.newName(
			rule.id.name + (args.length ? "<" + args.join(",") + ">" : ""),
			nodeName || true,
			props,
		);
	...
	nodeInfo(
		...
		args: readonly Expression[] = none,
		params: readonly Identifier[] = none,
		expr?: Expression,
		defaultProps?: Props,
	): {
		name: string | null;
		...
		let result: Props = {};
		let name =
			defaultName && (allow.indexOf("a") > -1 || !ignored(defaultName)) && !/ /.test(defaultName)
				? defaultName
				: null;
		let dialect = null,
			dynamicPrec = 0,
		...
		for (let prop of props) {
			if (!prop.at) {
				if (!this.knownProps[prop.name]) {
					let builtin = ["name", "dialect", "dynamicPrecedence", "export", "isGroup"].includes(
						prop.name,
					)
	...
	finishProp(prop: Prop, args: readonly Expression[], params: readonly Identifier[]): string {
		return prop.value
			.map((part) => {
				...
				if (pos < 0)
					this.raise(
						`Property refers to '${part.name}', but no parameter by that name is in scope`,
						part.start,
					);
				let expr = args[pos];
				...
				return this.raise(
					`Expression '${expr}' can not be used as part of a property value`,
					part.start,
				);
			})
			.join("");
	...
	resolveSpecialization(expr: SpecializeExpression) {
		...
		if ((lit = isLiteralToken(expr.content)) != null) values = [lit];
		else if (
			expr.content instanceof ChoiceExpression &&
			expr.content.exprs.every((e) => isLiteralToken(e) != null)
		)
			values = expr.content.exprs.map(isLiteralToken) as string[];
		else
			return this.raise(
				`The second argument to '${expr.type}' must be a literal or choice of literals`,
				expr.content.start,
			);
		...
		for (let value of values) {
			...
			if (known == null) {
				...
				if (known.type != type)
					this.raise(
						`Conflicting specialization types for ${JSON.stringify(value)} of ${term.name} (${type} vs ${known.type})`,
						expr.start,
					);
				if (known.dialect != dialect)
					this.raise(
						`Conflicting dialects for specialization ${JSON.stringify(value)} of ${term.name}`,
						expr.start,
					);
				if (known.name != name)
					this.raise(
						`Conflicting names for specialization ${JSON.stringify(value)} of ${term.name}`,
						expr.start,
					);
				if (token && known.term != token)
					this.raise(
						`Conflicting specialization tokens for ${JSON.stringify(value)} of ${term.name}`,
						expr.start,
					);
				token = known.term;
			}
	...
	findDelimiters(expr: Expression) {
		...
		const brackets = ["()", "[]", "{}", "<>"];
		let bracket = brackets.find(
			(b) => lastToken!.str.indexOf(b[1]) > -1 && lastToken!.str.indexOf(b[0]) < 0,
		);
		if (!bracket) return null;
		let firstToken = findToken(expr.exprs[0]);
		if (
			!firstToken ||
			!firstToken.term.nodeName ||
			firstToken.str.indexOf(bracket[0]) < 0 ||
			firstToken.str.indexOf(bracket[1]) > -1
		)
			return null;
	...
	defineGroup(rule: Term, group: string, ast: RuleDeclaration) {
		...
		let getNamed = (rule: Term): Term[] => {
			...
			if (recur.includes(rule))
				this.raise(
					`Rule '${ast.id.name}' cannot define a group because it contains a non-named recursive rule ('${rule.name}')`,
					ast.start,
				);
			let result: Term[] = [];
			recur.push(rule);
			for (let r of this.rules)
				if (r.name == rule) {
					...
					if (names.length > 1)
						this.raise(
							`Rule '${ast.id.name}' cannot define a group because some choices produce multiple named nodes`,
							ast.start,
						);
	...
	checkGroups() {
		let groups: { [name: string]: Term[] } = Object.create(null),
			nodeNames: { [name: string]: boolean } = Object.create(null);
		...
		for (let i = 0; i < names.length; i++) {
			...
			for (let j = i + 1; j < names.length; j++) {
				...
				if (
					terms.some((t) => other.includes(t)) &&
					(terms.length > other.length
						? other.some((t) => !terms.includes(t))
						: terms.some((t) => !other.includes(t)))
				)
					this.warn(
						`Groups '${name}' and '${names[j]}' overlap without one being a superset of the other`,
					);
			}
		}
	}
...
class FinishStateContext {
	...
	findSharedActions(state: LRState): SharedActions | null {
		...
		for (let shared of this.sharedActions) {
			if (
				(!found || shared.actions.length > found.actions.length) &&
				shared.actions.every((a) => state.actions.some((b) => b.eq(a)))
			)
				found = shared;
		}
		if (found) return found;
		let max: (Shift | Reduce)[] | null = null,
			scratch = [];
		for (let i = state.id + 1; i < this.states.length; i++) {
			let other = this.states[i],
				fill = 0;
	...
	storeActions(
		actions: readonly (Shift | Reduce)[],
		skipReduce: number,
		shared: SharedActions | null,
	) {
	...
	finish(state: LRState, isSkip: boolean, forcedReduce: number) {
		...
		let skipTable = this.skipData[skipID],
			skipTerms = this.skipInfo[skipID].startTokens;
		...
		for (let i = 0; i < state.actions.length + skipTerms.length; i++) {
			let term =
				i < state.actions.length ? state.actions[i].term : skipTerms[i - state.actions.length];
			for (;;) {
		...
		for (let i = 0; i < this.tokenizers.length; i++) {
			...
			if (external.includes(tok) || tok.groupID == state.tokenGroup) tokenizerMask |= 1 << i;
		}

		let base = state.id * ParseState.Size;
		this.stateArray[base + ParseState.Flags] = flags;
		this.stateArray[base + ParseState.Actions] = this.storeActions(
			defaultReduce ? none : state.actions,
			skipReduce,
			shared,
		);
...
function buildSpecializeTable(spec: { value: string; term: Term; type: string }[]): {
	[value: string]: number;
} {
...
class TokenSet {
	...
	getToken(expr: NameExpression) {
		...
		let {
			...
			name,
			expr.args,
			rule.params.length != expr.args.length ? none : rule.params,
		);
	...
	buildRule(
		...
		this.build(
			this.b.substituteArgs(rule.expr, expr.args, rule.params),
			start,
			to,
				...
				for (let item of group.items) {
					...
					if (item instanceof NameExpression) {
						...
						let id = JSON.stringify(item.value),
							found = this.built.find((b) => b.id == id);
	...
	buildPrecTable(softConflicts: readonly Conflict[]) {
		let precTable: number[] = [],
			rel = this.precedenceRelations.slice();
...
class MainTokenSet extends TokenSet {
	...
	buildTokenGroups(states: readonly LRState[], skipInfo: readonly SkipInfo[], startID: number) {
		...
		if (tokens.accepting.length)
			this.b.raise(
				`Grammar contains zero-length tokens (in '${tokens.accepting[0].name}')`,
		...
		let allConflicts = tokens
			.findConflicts(checkTogether(states, this.b, skipInfo))
		...
		let softConflicts = allConflicts.filter((c) => c.soft),
			conflicts = allConflicts.filter((c) => !c.soft);
		...
		for (let state of states) {
			...
			for (let i = 0; i < state.actions.length + (skip ? skip.length : 0); i++) {
				let term =
					i < state.actions.length ? state.actions[i].term : skip[i - state.actions.length];
			...
			for (let term of stateTerms) {
				for (let conflict of conflicts) {
					let conflicting =
						conflict.a == term ? conflict.b : conflict.b == term ? conflict.a : null;
					if (!conflicting) continue;
					if (stateTerms.includes(conflicting) && !errors.some((e) => e.conflict == conflict)) {
						let example = conflict.exampleA
		...
		if (groups.length + startID > 16)
			this.b.raise(
				`Too many different token groups (${groups.length}) to represent them as a 16-bit bitfield`,
			);
...
class LocalTokenSet extends TokenSet {
	...
	getToken(expr: NameExpression) {
		...
		if (term && !this.b.tokenOrigins[term.name]) this.b.tokenOrigins[term.name] = { group: this };
		return term;
	}
	...
	buildLocalGroup(
		...
		for (let { a, b, exampleA } of tokens.findConflicts(() => true)) {
			if (!this.precededBy(a, b) && !this.precededBy(b, a))
				this.b.raise(
					`Overlapping tokens ${a.name} and ${b.name} in local token group${
		...
		for (let state of states) {
			...
			if (usesThis) {
				if (usesOther)
					this.b.raise(
						`Tokens from a local token group used together with other tokens (${
							usesThis.name
						} with ${usesOther.name})`,
					);
				state.tokenGroup = id;
...
function checkTogether(states: readonly LRState[], b: Builder, skipInfo: readonly SkipInfo[]) {
	...
	function hasTerm(state: LRState, term: Term) {
		return (
			state.actions.some((a) => a.term == term) ||
...
function rangeEdges(from: State, to: State, low: number, hi: number) {
	...
	let lowStr = String.fromCodePoint(low),
		hiStr = String.fromCodePoint(hi - 1);
	let lowA = lowStr.charCodeAt(0),
		lowB = lowStr.charCodeAt(1);
	let hiA = hiStr.charCodeAt(0),
		hiB = hiStr.charCodeAt(1);
	if (lowA == hiA) {
		// Share the first char code
		let hop = new State();
	...
	checkConflicts(states: readonly LRState[], skipInfo: readonly SkipInfo[]) {
		...
		for (let id of this.ast.conflicts) {
			...
			if (!term) {
				...
				this.b.warn(
					`External token set specifying a conflict with one of its own tokens ('${id.name}')`,
				);
			} else {
...
class ExternalSpecializer {
	...
	finish() {
		...
		if (terms.length != 1 || terms[0].terms.length != 1 || !terms[0].terms[0].terminal)
			this.b.raise(
				`The token expression to '@external ${this.ast.type}' must resolve to a token`,
				this.ast.token.start,
			);
...
function inlineRules(rules: readonly Rule[], preserve: readonly Term[]): readonly Rule[] {
	for (let pass = 0; ; pass++) {
		let inlinable: { [name: string]: readonly Rule[] } = Object.create(null),
			found;
		if (pass == 0)
		...
		for (let i = 0; i < rules.length; i++) {
			let rule = rules[i];
			if (
				!rule.name.interesting &&
				!rule.parts.includes(rule.name) &&
				rule.parts.length < 3 &&
				!preserve.includes(rule.name) &&
				(rule.parts.length == 1 ||
					rules.every((other) => other.skip == rule.skip || !other.parts.includes(rule.name))) &&
...
function mergeRules(rules: readonly Rule[]): readonly Rule[] {
	let merged: { [name: string]: Term } = Object.create(null),
		found;
	...
	for (let rule of rules)
		if (!merged[rule.name.name]) {
			newRules.push(
				rule.parts.every((p) => !merged[p.name])
					? rule
					: new Rule(
							rule.name,
							rule.parts.map((p) => merged[p.name] || p),
							rule.conflicts,
							rule.skip,
						),
			);
...
export function buildParserFile(
	text: string,
	options: BuildOptions = {},
): { parser: string; terms: string } {

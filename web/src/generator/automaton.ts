...
export class Pos {
	...
	cmp(pos: Pos): number {
		return (
			...
			this.pos - pos.pos ||
			this.skipAhead.hash - pos.skipAhead.hash ||
			cmpSet(this.ahead, pos.ahead, (a, b) => a.cmp(b)) ||
			cmpSet(this.ambigAhead, pos.ambigAhead, cmpStr)
		);
	}
	...
	eq(other: Pos): boolean {
		return (
			...
			(this.hash == other.hash &&
				...
				this.pos == other.pos &&
				this.skipAhead == other.skipAhead &&
...
export class Shift {
	...
	cmp(other: Shift | Reduce): number {
		return other instanceof Reduce
			? -1
...
export class Reduce {
	...
	cmp(other: Shift | Reduce): number {
		return other instanceof Shift
			? 1
			: this.term.id - other.term.id ||
					this.rule.name.id - other.rule.name.id ||
...
export class State {
	...
	toString(): string {
		...
		return (
			this.id +
			": " +
			this.set.filter((p) => p.pos > 0).join() +
			(this.defaultReduce
	...
	addActionInner(value: Shift | Reduce, positions: readonly Pos[]): Shift | Reduce | null {
		check: for (let i = 0; i < this.actions.length; i++) {
			...
			if (action.term == value.term) {
				...
				let fullPos = Pos.addOrigins(positions, this.set),
					actionFullPos = Pos.addOrigins(this.actionPositions[i], this.set);
				let conflicts = conflictsAt(fullPos),
					actionConflicts = conflictsAt(actionFullPos);
				let diff =
					compareRepeatPrec(fullPos, actionFullPos) ||
					conflicts.precedence - actionConflicts.precedence;
				if (diff > 0) {
					// Drop the existing action
	...
	for (let pos of set) {
		...
		if (next && !next.terminal)
			addFor(
				...
				pos.pos == pos.rule.parts.length - 1 ? pos.skipAhead : pos.rule.skip,
				pos,
			);
	}
	while (redo.length) {
...
function findConflictOrigin(a: Pos, b: Pos) {
	...
	function via(root: Pos, start: Pos) {
		...
		return hist
			.reverse()
			.map((p, i) => "\n" + "  ".repeat(i + 1) + (p == start ? "" : "via ") + p)
			.join("");
	}

	for (let p: Pos | null = a; p; p = p.via)
		for (let p2: Pos | null = b; p2; p2 = p2.via) {
...
function findConflictShiftSource(
	conflictPos: Pos,
	termAfter: Term,
	first: { [name: string]: (Term | null)[] },
) {
	...
	function explore(pos: Pos, i: number, hasMatch: Pos | null): string {
		...
		for (let next; (next = pos.next); ) {
			...
			let after = pos.rule.parts[pos.pos + 1],
				match = pos.pos + 1 == pos.rule.parts.length ? hasMatch : null;
...
export function buildFullAutomaton(
	terms: TermSet,
	startTerms: Term[],
	first: { [name: string]: (Term | null)[] },
): State[] {
	...
	function getState(core: readonly Pos[], top?: Term) {
		...
		let hash = hashPositions(set),
			forHash = statesBySetHash[hash] || (statesBySetHash[hash] = []);
		let found;
	...
	for (const startTerm of startTerms) {
		...
		getState(
			startTerm.rules.map((rule) => new Pos(rule, 0, [terms.eof], none, startSkip, null).finish()),
			startTerm,
		);
	}
	...
	for (let filled = 0; filled < states.length; filled++) {
		...
		for (let i = 0; i < byTerm.length; i++) {
			let term = byTerm[i],
				positions = byTermPos[i].map((p) => p.advance());
			if (term.terminal) {
	...
	for (let action of a.actions) {
		...
		if (setB && setB.some((other) => !other.matches(action, mapping))) {
			...
			if (
				...
				setA.some((a1) => !setB.some((a2) => a1.matches(a2, mapping)))
			)
				return false;
		}
	}
...
function collapseAutomaton(states: readonly State[]): readonly State[] {
	...
	assignGroups: for (let i = 0; i < states.length; i++) {
		...
		if (!state.startRule)
			for (let j = 0; j < groups.length; j++) {
				let group = groups[j],
					other = states[group.members[0]];
				if (
					state.tokenGroup == other.tokenGroup &&
	...
	for (let pass = 1; ; pass++) {
		...
		if (timing)
			console.log(
				`Collapse pass ${pass}${conflicts ? `` : `, done`} (${((Date.now() - t0) / 1000).toFixed(2)}s)`,
			);
		...
		for (let i = 0; i < states.length; i++) {
			...
			if (match < 0) {
				...
				let other = newStates[match],
					add: Pos[] | null = null;
				for (let pos of state.set)
					if (!other.set.some((p) => p.eqSimple(pos))) (add || (add = [])).push(pos);
				if (add) other.set = add.concat(other.set).sort((a, b) => a.cmp(b));
			}
		}
		if (timing)
			console.log(
				`Merge identical pass ${pass}${didMerge ? "" : ", done"} (${((Date.now() - t0) / 1000).toFixed(2)}s)`,
			);

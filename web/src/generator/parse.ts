...
function parseExprInner(input: Input): Expression {
	let start = input.start;
	if (input.eat("(")) {
		if (input.eat(")")) return new SequenceExpression(start, none, [none, none]);
		let expr = parseExprChoice(input);
		input.expect(")");
		return expr;
		...
		let content = input.value,
			invert = input.string[input.start] == "!";
		let unescaped = readString(
			content.replace(/\\.|-|"/g, (m: string) => {
		...
		if (input.eat(",")) {
			...
			input.raise(
				`@${value} requires two arguments when its first argument isn't a literal string`,
			);
		}
		input.expect(">");
	...
	do {
		...
		for (;;) {
			let localStart = input.start,
				markerType!: "ambig" | "prec";
			if (input.eat("~")) markerType = "ambig";
			else if (input.eat("!")) markerType = "prec";
			else break;
			markers[markers.length - 1] = markers[markers.length - 1].concat(
				new ConflictMarker(localStart, parseIdent(input), markerType),
			);
		}
...
function parseTokenPrecedence(input: Input) {
	...
	while (!input.eat("}")) {
		...
		let expr = parseExprInner(input);
		if (expr instanceof LiteralExpression || expr instanceof NameExpression) tokens.push(expr);
...
function parseExternalSpecialize(input: Input, type: "extend" | "specialize", start: number) {
	...
	input.expect("id", "from");
	let from = input.expect("string");
	return new ExternalSpecializeDeclaration(
		start,
		type,
		token,
		id,
		from,
		parseExternalTokenSet(input, false).tokens,
	);
}
...
function readString(string: string) {
	let point = /\\(?:u\{([\da-f]+)\}|u([\da-f]{4})|x([\da-f]{2})|([ntbrf0])|(.))|[^]/giy;
	let out = "",
		m;

...
export interface NestedParse {
	...
	overlay?:
		...
		| ((node: SyntaxNodeRef) => { from: number; to: number } | boolean);
...
export function parseMixed(
	...
	return (parse, input, fragments, ranges): PartialParse =>
			...
			if (this.stoppedAt != null && cursor.from >= this.stoppedAt) {
				...
				if (typeof nest.overlay == "function") {
					...
					let ranges = punchRanges(
						this.ranges,
						nest.overlay || (cursor.from < cursor.to ? [new Range(cursor.from, cursor.to)] : []),
					);
...
function punchRanges(
	outer: readonly { from: number; to: number }[],
	ranges: readonly { from: number; to: number }[],
) {
	let copy: { from: number; to: number }[] | null = null,
		current = ranges;

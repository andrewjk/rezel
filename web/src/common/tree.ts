	...
	nextChild(
		...
		for (let parent: TreeNode = this; ; ) {
			for (
				...
				if (
					!(
						mode & IterMode.EnterBracketed &&
						next instanceof Tree &&
						(mounted = MountedTree.get(next)) &&
						!mounted.overlay &&
						mounted.bracketed &&
						pos >= start &&
						pos <= start + next.length
					) &&
	...
	enter(pos: number, side: -1 | 0 | 1, mode = 0) {
		...
		if (
			...
			for (let { from, to } of mounted.overlay) {
				if (
					(side > 0 || enterBracketed ? from <= rPos : from < rPos) &&
...
export class TreeCursor implements SyntaxNodeRef {
	...
	sibling(dir: 1 | -1) {
		...
		return d < 0
			? this.yield(
					this.buffer.parent.nextChild(this.buffer.index + dir, dir, 0, Side.DontCare, this.mode),
				)
			: false;
	}
